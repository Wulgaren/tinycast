...
- **Non-activating**, like the palette: the dialog takes key focus for its own keys without pulling app
  focus off whatever the user was in. It sits at `WindowLevels.dialog`, above every other Tinycast surface, and is
...
dialog takes, but sits at `WindowLevels.surface` rather than `WindowLevels.dialog` so a failure report still lands on
top of it.
