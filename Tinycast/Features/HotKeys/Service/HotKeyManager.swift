...
final class HotKeyManager {
    ...
    var onRunAppleShortcut: ((String) -> Void)?
    ...
    /// Shortcuts identifiers with a binding — same role as extension commands, own namespace.
    var boundAppleShortcutIDs: [String] {
        UserDefaults.standard.stringArray(forKey: boundAppleShortcutKey) ?? []
    }
    ...
    func setBinding(_ binding: HotKeyBinding?, for action: HotKeyAction) {
        ...
        case .appleShortcut(let id):
            var set = Set(boundAppleShortcutIDs)
            if binding == nil { set.remove(id) } else { set.insert(id) }
            UserDefaults.standard.set(Array(set), forKey: boundAppleShortcutKey)
